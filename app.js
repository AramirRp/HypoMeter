...
const TWITCH_REDIRECT_URI = 'http://localhost:5500';
...
async function initTwitchClient() {
    ...
    try {
        ...
        document.getElementById('connectionText').textContent = 'Erreur de connexion';
    }
}
...
function startNewGameContext() {
    ...
    console.log('Nouveau contexte démarré - les messages sont maintenant capturés');
}
...
function simulateChatMessage() {
    ...
    const viewers = ['Viewer123', 'Gamer456', 'HypeGamer', 'StreamFan', 'GameLover'];
...
function analyzeMessage(message) {
    ...
    if (foundKeywords.length > 0) {
        const positiveCount = foundKeywords.filter(k => k.type === 'positive').length;
        const negativeCount = foundKeywords.filter(k => k.type === 'negative').length;
...
// Ajout d'un mot-clé
function addKeyword() {
    const newKeyword = document.getElementById('newKeyword').value.trim().toLowerCase();
        ...
        gameDiv.innerHTML = `
            ...
            <div class="game-info">
                ...
                <div class="manual-hype-controls" style="margin-top:8px;">
                    <button class="btn-hype" data-gameid="${game.id}" data-type="plus">+1</button>
                    <button class="btn-dehype" data-gameid="${game.id}" data-type="minus">-1</button>
                </div>
            </div>
        `;
...
function updateUI() {
    ...
    if (state.activeGame && !state.activeGame.isPending) {
        document.getElementById('currentGameName').textContent = state.activeGame.name;
        document.getElementById('activeGameContext').style.display = 'block';
        document.getElementById('newGameContext').style.display = 'none';
        document.getElementById('gameInputForm').style.display = 'none';
        document.getElementById('pendingGameContext').style.display = 'none';
...
function updateTwitchUI() {
    ...
    document.getElementById('twitchConnectBtn').style.display = isConnected ? 'none' : 'inline-block';
    document.getElementById('twitchUserInfo').style.display = isConnected ? 'block' : 'none';
    document.getElementById('channelInputs').style.display = isConnected ? 'block' : 'none';
    
    if (isConnected) {
        document.getElementById('twitchUsernameDisplay').textContent = `Connecté en tant que ${username}`;
